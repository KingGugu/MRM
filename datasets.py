...
class FoldDataset(Dataset):
    def __init__(self, root='/protein-data/fold', random_seed=0, split='training'):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "training":
            sampled_pos, sampled_ori, sampled_amino, sampled_seq = self.augmentation(
                pos, ori, amino
            )
        else:
            sampled_pos, sampled_ori, sampled_amino, sampled_seq = pos, ori, amino, seq

        data_aug = Data(
            x=torch.from_numpy(sampled_amino),
            edge_index=None,
            edge_attr=None,
            y=label,
            ori=torch.from_numpy(sampled_ori.astype(np.float32)),
            seq=torch.from_numpy(sampled_seq.astype(np.float32)),
            pos=torch.from_numpy(sampled_pos.astype(np.float32))
        )
...
class FuncDataset(Dataset):
    ...
    def __init__(self, root='/protein-data/func', random_seed=0, split='training'):
...
class GODataset(Dataset):
    ...
    def __init__(self, root='/protein-data/go', level='mf', percent=30, random_seed=0, split='train'):
...
class ECDataset(Dataset):
    ...
    def __init__(self, root='/protein-data/ec', percent=30, random_seed=0, split='train'):
