...
class Model(nn.Module):
    ...
    def forward(self, data, aug_data=None, mix_weight=None):
        if aug_data is not None:
            x, pos, seq, ori, batch = (self.embedding(data.x), data.pos, data.seq, data.ori, data.batch)

            aug_x, aug_pos, aug_seq, aug_ori, aug_batch = (
                self.embedding(aug_data.x), aug_data.pos, aug_data.seq, aug_data.ori, aug_data.batch)

            for i, layer in enumerate(self.layers):
                x = layer(x, pos, seq, ori, batch)
                aug_x = layer(aug_x, aug_pos, aug_seq, aug_ori, aug_batch)

                if i == len(self.layers) - 1:
                    x = global_mean_pool(x, batch)
                    aug_x = global_mean_pool(aug_x, aug_batch)
                    aug_x = x * (1 - mix_weight) + aug_x * mix_weight
                elif i % 2 == 1:
                    x, pos, seq, ori, batch = self.local_mean_pool(x, pos, seq, ori, batch)
                    aug_x, aug_pos, aug_seq, aug_ori, aug_batch = self.local_mean_pool(aug_x, aug_pos, aug_seq, aug_ori,
                                                                                       aug_batch)

            out = self.classifier(x)
            out_aug = self.classifier(aug_x)
            return out, out_aug
